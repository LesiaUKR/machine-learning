...
# Додавання стовпця "Age" для віку і його обчислення
train_data['Age'] = calculate_age(train_data['Date_Of_Birth'])
valid_data['Age'] = calculate_age(valid_data['Date_Of_Birth'])

# %% Видалення стовпця "Date_Of_Birth", оскільки тепер є стовпець "Age"
...
cat_cols = ['Qualification', 'Role', 'Cert']
...
valid_data = pd.concat([valid_data.drop(categorical_cols, axis=1), valid_encoded], axis=1)


# %% Розділення на ознаки (X) та цільову змінну (y)
X_train = train_data.drop(['Salary', 'Age'], axis=1)
y_train = train_data['Salary']
X_valid = valid_data.drop(['Salary', 'Age'], axis=1)
...
# # %% Обчислення метрики MAPE (середня абсолютна помилка у відсотках)
# mape = mean_absolute_percentage_error(valid_data['Salary'], y_valid_pred)
# print(f'Validation MAPE: {mape:.2%}')

# from sklearn.model_selection import GridSearchCV

# # Налаштування гіперпараметрів для пошуку найкращого значення k
# param_grid = {'n_neighbors': np.arange(1, 20)}
# knn = KNeighborsRegressor()

# # Використання GridSearchCV для пошуку оптимального параметра k
# knn_gscv = GridSearchCV(knn, param_grid, cv=5, scoring='neg_mean_absolute_percentage_error')
# knn_gscv.fit(X_train_scaled, y_train)

# # Отримання найкращого значення k
# best_k = knn_gscv.best_params_['n_neighbors']
# print(f"Найкраще значення k: {best_k}")
